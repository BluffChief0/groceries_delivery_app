...
async def set_nutritional_and_package(
    product_id: str,
    proteins: float,
    fats: float,
    carbohydrates: float,
    ...
    append: bool = False,  # False = перезаписать список; True = добавить в конец
) -> bool:
    async with AsyncSessionLocal() as s:
        prod = await s.scalar(select(Product).where(Product.id == product_id))
        if not prod:
            return False
        ...
        if append:
            if not prod.nutritional:
                prod.nutritional = []
            prod.nutritional.append(item)
        else:
            prod.nutritional = [item]

        if package is not None:
            prod.package = package

        if composition is not None:  # опционально, если захочешь заполнить 9-ю колонку
...
sets = ["Пластиковая упаковка", "Картонная упаковка", "Бумажный пакет", "Сетчатый пакет", "Стеклянная бутылка", "Ж/б консерва"]


async def main():
    ok = await set_nutritional_and_package(
        "ab4d76b2-4c54-4577-bba2-89720dd2913e",
        proteins=round(random.uniform(0.1, 10.0), 1),
        fats=round(random.uniform(0.0, 6.0), 1),
        carbohydrates=round(random.uniform(1.0, 20.0), 1),
        package=sets[3],
    )
    print("OK" if ok else "Product not found")
    

VALUE = "Описание состава продукта."

async def fill_composition_all(value: str = VALUE) -> int:
    async with AsyncSessionLocal() as s:
        # Вариант 1: ORM update (коротко)
        res = await s.execute(update(Product).values(composition=value))
        await s.commit()
        return res.rowcount or 0  # иногда может вернуть -1 в SQLite

        # Вариант 2 (если нужен «точный» rowcount на SQLite):
        # res = await s.execute(text("UPDATE products SET composition = :v"), {"v": value})
        # await s.commit()
        # return res.rowcount or 0

async def bane():
    n = await fill_composition_all()
